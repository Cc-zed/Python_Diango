...
from .serializers import PasswordResetRequestSerializer, PasswordResetConfirmSerializer, UserProfileSerializer
...
class GoogleLoginView(APIView):
    ...
    def post(self, request):
        ...
        # Якщо у користувача немає аватара, завантажити з Google
        if avatar_url and (not user.image or not user.image.name):
            try:
                image_content = urlopen(avatar_url).read()
                ext = avatar_url.split('.')[-1].split('?')[0][:4]
                file_name = f"google_avatar_{user.pk}.{ext}"
                user.image.save(file_name, ContentFile(image_content), save=True)
            except Exception as e:
                pass  # можна залогувати помилку
...
class UserProfileView(APIView):
    ...
    def put(self, request):
        serializer = UserProfileSerializer(request.user, data=request.data, partial=False, context={"request": request})
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
